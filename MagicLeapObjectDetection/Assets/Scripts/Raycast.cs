...
public class Raycast : MonoBehaviour
{
    ...
    public void StartCast(MLRaycast.QueryParams raycastParams, string text, int material)
    {
        ...
        MLRaycast.Raycast(raycastParams, 
            (state, point, normal, confidence) => HandleOnReceiveRaycast(text, material, state, point, normal, confidence));
    }
    void HandleOnReceiveRaycast(string text, int material, MLRaycast.ResultState state,
        UnityEngine.Vector3 point, Vector3 normal, float confidence)
    {
